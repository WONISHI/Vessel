...
export default function MarkdownToolbar({
  ...
  const handleFormatBrush = () => {
    if (copiedStyle) {
      // 应用样式
      if (copiedStyle.bold) editor.chain().focus().setBold().run()
      else editor.chain().focus().unsetBold().run()
      if (copiedStyle.italic) editor.chain().focus().setItalic().run()
      else editor.chain().focus().unsetItalic().run()
      if (copiedStyle.strike) editor.chain().focus().setStrike().run()
  ...
  const items: MarkdownToolbarItem[] = [
    ...
    {
      label: "Insert Media (Image/Video)",
      icon: ImageIcon,
